...
const REPORT_TYPES = [
  ...
  { id: "lost_item" as ReportType, label: "Lost Item", icon: "help-circle", color: "#FF9500", description: "Report a lost item" },
  { id: "missing_pet" as ReportType, label: "Missing Pet", icon: "paw", color: "#34C759", description: "Report a missing pet" },
  { id: "hazard" as ReportType, label: "Hazard", icon: "alert-circle", color: "#FFCC00", description: "Report environmental hazards" },
];
...
export default function CreateReportScreen() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    console.log(data, "-----form data-----")
      ...
      const metadata: any = {};
      if (selectedType === "lost_item") metadata.lost_item = { itemType: data.itemType, color: data.color };
      if (selectedType === "missing_pet") metadata.missing_pet = { species: data.species, breed: data.breed, color: data.color };
      if (selectedType === "crime") metadata.crime = { category: data.crimeCategory };
      if (selectedType === "hazard") metadata.hazard = { type: data.hazardType };
        ...
        <View style={styles.typeGrid}>
          {REPORT_TYPES.map((t) => (
            <TouchableOpacity
              ...
              onPress={() => { setSelectedType(t.id); setValue("type", t.id); }}
            >
              <View style={[styles.typeIconContainer, { backgroundColor: t.color }]}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.formHeader}>
          ...
          <View style={styles.formHeaderText}>
            <View style={[styles.formHeaderIcon, { backgroundColor: selectedTypeData?.color }]}>
              <Ionicons name={selectedTypeData?.icon as any} size={20} color="#fff" />
            </View>
            <Text style={styles.formHeaderTitle}>{selectedTypeData?.label}</Text>
          </View>
        </View>
          ...
          <View style={styles.photosContainer}>
            {photos.map((p, i) => (
              <View key={i} style={styles.photoItem}>
                ...
                <TouchableOpacity onPress={() => removePhoto(i)} style={styles.removePhotoButton}>
        ...
        <View style={styles.section}>
          ...
          >
            {locationLoading ? (
              ...
              <>
                <Ionicons name={location ? "checkmark-circle" : "location"} size={24} color={location ? "#34C759" : "#007AFF"} />
          ...
          <View style={styles.switchRow}>
            <View style={styles.switchLeft}>
              ...
              <View>
                ...
                <Text style={styles.switchSublabel}>Your name won’t be shown</Text>
              </View>
            </View>
            <Controller
              control={control}
              name="isAnonymous"
              render={({ field: { onChange, value } }) => (
                <Switch value={value} onValueChange={onChange} thumbColor={value ? "#007AFF" : "#f4f3f4"} />
              )}
            />
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  typeCard: { flexBasis: "47%", borderWidth: 2, borderRadius: 12, padding: 16, alignItems: "center" },
  typeIconContainer: { width: 50, height: 50, borderRadius: 25, justifyContent: "center", alignItems: "center", marginBottom: 10 },
  ...
  formHeaderIcon: { width: 28, height: 28, borderRadius: 14, justifyContent: "center", alignItems: "center", marginRight: 8 },
  formHeaderTitle: { fontWeight: "700", fontSize: 18 },
  section: { marginBottom: 24 },
  label: { fontWeight: "600", marginBottom: 8 },
  required: { color: "#FF3B30" },
  textArea: { borderWidth: 1, borderColor: "#ccc", borderRadius: 8, padding: 12, minHeight: 120, textAlignVertical: "top" },
  ...
  locationButton: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: "#ccc", padding: 12, borderRadius: 8 },
